...
- **写真・図はイベントのフォルダに置き、相対パスで指す。** ファイル名はASCIIにする
  （日本語名と空白はURLで壊れる）。寸法は固定pxで持たず `width:100%` と `max-width` で
  親に従わせる。原寸のまま置くと393px幅からはみ出す。**現地で開く図に `loading="lazy"`
  を付けない。** 通信が無いときに空のまま残る。小さく載る図は、画像そのものを
  ファイルへのリンクで包んで原寸で開けるようにする。**配る画像も、リンクの先が在ることと
  gitに無視されていないことを対で検査する**（HRSの `validate.mjs` は生成物が指す
  同フォルダのファイルをすべて集めて `git check-ignore` に渡す）。
