...
**概要の区分は1本の軸で付ける。** 「その日の行為」（移動・仕事）と「旅程上の位置」
（出国・到着・帰路・帰国）を混ぜない。HRSは2026-08-23まで9/8が「移動」で9/13が
「帰国日」だった。行為で名付けると往路の到着日と復路の出発日がどちらも「移動」になり、
区分から向きが消える。境目の日は越える境界で呼び、間の日は中身で呼ぶ。
