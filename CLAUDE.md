...
**移動だけは概要でも1日1行にしない。** 1区間1行で、出発時刻・地点・発、到着時刻・
地点・着、便名、所要を出す。値は旅程の区間データから引き、概要のために書き直さない。
**フライトだけにしない。** 都市間の陸路も同じ形で出す（HRSの9/8はフランクフルト着の
あとICEでシュトゥットガルトへ入るので、フライトだけでは移動が半分しか出ない）。
空港アクセスの近距離移動は当日の行動計画の粒度なので旅程が持つ。案によって便も時刻も
変わる区間は、案が決まるまで出さない。**時刻帯は、出発と到着で違うときだけ出す**
（0:45発・19:35着で所要12時間50分の区間は時刻帯が無いと数字が合わない。同じ時刻帯の
中を走る列車には要らない）。日付は、またぐ側にだけ添える。地点は、空港はコード、
駅は駅名。同じ場所が2つの区間行で違う字にならないようにする。

**その日の中身は時刻順に並べる。** 主な内容を先頭に固定すると、9/8が
「Stuttgart Hbf着（12:11）→ 7:40のフライト」と逆順に出る。区間行も主な内容も
同じ並びに入れて時刻で並べ替える。時刻を持たない行（「日中」）はその日の先頭に置く。
並べる日は出発日で決める。HRSの帰国便は家族向けでは前夜の続きとして書いてあるが、
実際の出発は日付をまたいだ9/14 0:45で、9/13へ寄せると帰国の1日が地上の予定だけに見える。
代表イベントの選び方も対で直す。**区間行が持っている移動を「主な内容」にも出さない。**
1行に同じ事実が2回並ぶ。区間行にしていない移動（空港アクセス）は主な内容に残す。
