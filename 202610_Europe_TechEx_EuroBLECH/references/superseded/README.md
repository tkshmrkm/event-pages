...
`../../index.html` が `../../build_v3.mjs` の生成物であり、フォルダの公開入口である。
`../../index_v3.html` は以前の共有URLを `./` へ転送する互換ページで、生成物ではない。
...
一方で、視察テーマとセッション選定の中身は `index.html` にも `index_v3.html` にも
...
`../../CLAUDE_HANDOFF.md` の編集ルールに従って `build_v3.mjs` 側で反映する。
