...
### Tailwind風マークアップ — 保留（実測だけ残す）

**採用も全廃も決めていない。** 判断材料は下記（2026-08-15、絵文字全廃後の実測）。

- クラス種類 **105**。うち **40種は `v3.css` 側に受けがあり効いている**
  （`bg-white` `flex` `rounded-xl` `grid-cols-2` `text-slate-600` など）。
  レイアウトの骨格を担っているので、消すと崩れる
- **65種が無効**（延べ454回）。`py-0.5` `bg-slate-100` `hover:bg-slate-200`
  `border-l-4` `text-white` など。消しても見た目は1pxも変わらない
- HRSも最初はTailwind風だった（`index_v1.html` は739トークン、`index_v2.html` で0）。
  v1→v2の書き直しで外れた。**外した理由はリポジトリのどこにも記録が無い。**

**混在の実害が2件出ている。どちらも2026-08-15に踏んだ。**

1. **日付バッジが濃い地に黒文字だった。** 背景はインラインstyleなので効き、文字色は
   `text-white` というクラス名なので効かない。書いた側は指定したつもりでいた。
   インライン背景を持つ要素だけに色を当てて直した。**クラス指定の13件は触らない**
   （背景も効いていないので、白にすると白地に白文字になる）。
   ティールの `#0d9488` は白文字とのコントラストが3.7:1しかなかったので `#0B6E66` へ寄せた。
