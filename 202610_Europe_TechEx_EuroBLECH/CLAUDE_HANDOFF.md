...
更新日: 2026-08-14（旅程・家族タブの作り込みまで完了）
...
### 1. ラウンジ情報の集約 — 方針は決まったが未実施

準備タブに「ラウンジ利用可否」の表がある。ラウンジは**空港で過ごす選択肢**であり、
会場タブ（展示会場・工場の情報）とは性質が違うので、**準備タブが置き場所として正しい**。

旅程には当日その場で要る最小限（4系統の名前と営業時間）だけ残し、
資格の確認は準備タブへ誘導する。10/24だけ先行して「準備タブの
『ラウンジ利用可否』に集約してある」という誘導を入れた。**残りの日は未着手。**

### 2. 会場そのものへの地図リンク

交通行のendpointは降車地（`Hannover Messe/Laatzen` は駅）を指す。これは正しいので変えない。
会場そのもの（ハノーファーメッセ、RAI）へは別に導線が要る。10/21のEuroBLECH行にだけ
`会場はハノーファーメッセ` のリンクを張った。**他の会場日（10/20・10/23）は未着手。**
