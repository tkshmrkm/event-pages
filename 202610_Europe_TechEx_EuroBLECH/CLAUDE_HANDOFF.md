...
更新日: 2026-08-16（HRSの5タブ化で決まったことをEBへ戻す候補として追記）
...
## 2026-08-16にHRSで決まったこと — EBへ戻す候補

HRSを5タブ（`概要 / 旅程 / 視察 / 準備 / 記録`）にした日の成果。
**下の数値はこの日にEBの生成物を実際に数えた実測値。着手前に数え直すこと。**

### 1. 準備タブに現地用のものを置かない ← 最優先。EBは同じ罠を踏んでいる

HRSの準備タブには要対応タスク・利用フライト・宿泊・地図リンク集の4枚があった。
準備は出発前に埋め切ったら畳むタブなのに、後ろの3枚は現地で使う。畳めば
9/7の夜に空港で便を引けなくなる。3枚を旅程へ移し、準備を要対応タスク1枚だけにした。

**EBの `#tab-prep` は25KBあり、「ラウンジ利用の詳細」と
「香港（HKG）乗継の共通メモ — 無料シャワー・24時間営業・乗継の実務」を持つ。**
どちらも乗継中に見るもので、出発前に用が済むものではない。HRSと同じ整理が要る。
`validate_v3.mjs` に、準備へ現地用が戻ったら落ちる検査を入れること
（HRS側の実装が参考になる。準備に何が在り、旅程に何が在るかを対で見ている）。

### 2. `会場` → `視察` の改名

HRSは改名済み。**表示名だけ**で、内部キー（`data-tab="venue"` / `id="tab-venue"` /
ストレージの `ses:`）は変えていない。保存済みのタブ状態と記録が壊れるため。
EBの `>会場<` は1箇所。改名するなら同じ方針で。

理由は、そのタブに入力するのが「何を見に来たか」と「何を見たか」であって、
場所ではないから。展示会・講演会・工場見学・企業訪問を全部覆うので、
SIGGRAPH AsiaとLOGIMATでもそのまま使える。

**タブ名は2文字まで。** 5タブのHRSで実測したところ、4文字の「イベント」は393pxで
13px、5文字の「セッション」は26pxはみ出す。EBは4タブなのでもう少し余裕はあるが、
数えてから決めること。

### 3. `.flight-card` の死蔵CSSを落とす

**EBの `v3.css` に10規則あるが、`index.html` での使用は0。** 家族向けから外した
ときに規則だけ取り残された。実体は `shared/trip-field/core.css` へ移してHRSの
利用フライトが使っている。EB側の重複は削除してよい。

**EBで使いたい場合はcore.cssのほうを読む。** EBは復路が村上とチームで分かれるので、
1区間を「出発 → 便名 → 到着」で見せるカードは効きが大きいはず。
中央列は96pxしかないので、機材と所要だけを入れる。長い注記は `.flight-note` で
区間の下に全幅で出す。
...
EBは村上とチームで日程が割れるので、そのまま移せるかは要検討。
