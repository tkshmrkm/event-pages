...
Build a field-use version for smartphone operation, including possible use outdoors in direct sunlight. Preserve the useful overview and continuous scrolling of `index.html` (v1), while retaining the venue and recording capabilities developed in the hand-edited content source, now `source.html`.

## Deliverables

- `index.html`: primary online version and the folder's published entry point. Generated by
  `build.mjs`; never edit it directly.
- `desk_print.html`: self-contained desk-print version with no CDN or external runtime dependency.
...
The existing `index.html` and the content source (now `source.html`) remain unchanged at this checkpoint.

## Implementation status

Completed on 2026-08-13:

- `index.html`: primary field-use version with three-section navigation, continuously open day cards, outdoor-mobile palette, and separate candidate selection/confirmation states. Published as the folder's entry point.
- `desk_print.html`: self-contained static desk-print copy containing itinerary, preparation, and venue reference information.
- `family_print.html`: separate family-facing print document generated at day-level detail.
- `style.css`: shared stylesheet for the online field guide and family print page.
- `build.mjs`: generates all deliverables from the canonical content in `source.html` plus the presentation and transfer rules.
- `validate.mjs`: checks HTML balance, script syntax, desk-print independence, the three-section navigation, eight open day cards, JSON transfer controls, and the eight-row family summary.
...
Cloudflare synchronization was deployed on 2026-08-13 at `https://trip-field-sync.mrkn55.workers.dev`. The synchronization key is stored only as a Worker Secret and by the user; it is not present in the page source or repository. The public GitHub Pages UI was checked at smartphone and PC widths, and an authenticated PUT/GET round trip returned an identical payload. The cloud-enabled v3 workflow is complete and is published as the directory's canonical `index.html`. The former `index_v3.html` redirect page was deleted on 2026-08-19 when version numbers were removed from filenames.
