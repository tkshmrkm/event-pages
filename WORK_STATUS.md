...
- **同じ関数にしないことと、同じ規約に従わないことは別。** 2026-08-23にHRSで直した
  3つの穴は、EBの `build.mjs` にそのまま残っている。次にEBを触るときの候補。
  - 区分が `出国 / 帰国 / 帰国便 / イベント / 移動 / 休日 / 日本` で、暦（休日）と
    中身が同じ札に載っている。HRSの「帰国日」と同じ形
  - `OVERVIEW_MAIN_KIND_ORDER` で代表1件だけを選ぶので、移動が複数ある日はどれかが
    消える。`flight` を `move` より上にした理由のコメント（10/25の到着が消える）が
    そのまま残っていて、これはHRSでは区間行を出して解消した
  - 行の並びが種類の優先順で、時刻順ではない
