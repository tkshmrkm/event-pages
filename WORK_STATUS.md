...
- **1番（家族向け印刷版）と2番（家族タブをHRS5構成へ）を実施。**
  EBの生成物は `index.html` と `family_print.html` の2件になり、
  オンライン版のタブは5→4（`旅程 / 準備 / 会場 / 記録`）。
  `family-where` と死蔵データ `WHERE_DAYS` を削除し、フライト8便はサマリーの
  行き帰り4行へ畳んだ。`予約クラス` 8件はこれで0件になった（判断済み項目4も同時に完了）
- **6番（緊急連絡先の裏取り）を実施。** 各公館の公式ページを直接読んだ。
  `+49 30 21094-222` は緊急番号ではなく**FAX番号**だった。
  オランダ大使館の番地も `2` ではなく `5` が正しい。詳細はEB側の引き継ぎにある
- `validate_v3.mjs` は66件→73件。家族の検査は `family_print.html` を見る形へ移した

### 数え直しの教訓（2件目）

以前この節に「`📍 地図` の別リンク2件」と書き、次に「別リンクは0件で22件すべてが
リンク文の先頭に付いた `📍`」と直したが、**この2度目も誤りだった。**
完全一致で `>地図</a>` を探したため、先頭に `📍 ` が付いた5件が漏れていた。
実際は「地名に張ったリンク17件」と「リンク文が『地図』の別リンク5件」に分かれる。

**同じ箇所で2回続けて数え違えている。** 検索式が対象を捉えているかを、
件数ではなく実物を並べて確認すること。
