...
- **予定の状態を `.plan-state` へ** — HRSは9箇所そろえた。EUROBLECHは
  候補12・確定11・未定5・要検討2の語が散っている。共通部品は `core.css` にあるので、
  対応付けを決めて当てるだけ
- **絵文字の全廃** — HRSは3生成物とも0件にした。EUROBLECHは未着手。
  `build_v3.mjs` の `LINE_ICON_PATHS`（41種）と `EMOJI_ICON_MAP`、`dropFlags`、
  `stripScriptEmoji` をそのまま持っていける。国旗17件の項目はこれに含まれる
- `v3.css` の `.line-icon` 固定19pxの複製を削る — 共通定義（`core.css`、`1.15em`）に
  任せる。EUROBLECHの `.line-icon` は56件あり、そのうち44件が13〜14pxの文字の中で
  比1.36〜1.46になっている（2026-08-15に実機で計測）。色の上書きだけ残す
