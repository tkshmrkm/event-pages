...
検査は89件から97件へ。数値はすべてEBの生成物を数え直した実測値である。
...
1. **EBに概要タブを作るか。** HRSは `出張概要 / 日程概要 / イベント概要 / 施設概要`
   の4ブロックで、日程概要は日ごとの区分を `FAMILY_DAYS` から導出している。
   EBは村上とチームで日程が割れる（村上は10/17発でAMS経由、美馬・金築は10/18発で
   FRA経由、合流は10/20夜）ため、1日1区分の形がそのまま使えるかは未検討。
   タブは4→5になる。**ユーザーの判断待ち**
