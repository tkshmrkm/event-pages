...
- クラス種類105。**40種は効いている**（`v3.css` 側に受けがある。レイアウトの骨格）
- **65種が無効**（延べ454回）。消しても見た目は変わらない
- HRSも最初はTailwind風だった（v1で739トークン、v2で0）。**外した理由の記録は無い**

混在の実害が2件出た。日付バッジが濃い地に黒文字だった件（背景はインラインstyleで効き、
文字色は `text-white` で効かない）と、印刷版で背景色が全部消えた件
（色の規則が `.legacy-tab` に紐づくのに外枠ごと切り出していた）。どちらも修正済み。
