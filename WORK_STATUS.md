...
- HRS概要の日程概要（2026-08-23）。区分を1本の軸（`出国 / 到着 / イベント / 休日 /
  帰路 / 帰国`）にそろえ、フライトを1区間1行で出し、空港の呼び名を
  `中部国際空港` / `NGO` / `中部国際空港（NGO）` の3つに決めた。
  それまでは9/8が「移動」で9/13が「帰国日」と軸が割れ、帰国便のAY1416とAY79が
  概要のどこにも出ておらず、同じ画面に空港の呼び名が3通りあった
