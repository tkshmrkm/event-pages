...
- **2027年のPhysical AI系候補（2026-09-23）** … ローカルの未追跡ファイル
  ...
  - **方針とのずれ（未判断）：** 以前の方針では「TechExは1回だけ触れる扱い」「ACC・CDC・
    ASME IDETC/CIEのような特定分野の会議と、RSJのような国内学会は載せない」としていた。
    PR #4で入ったACC・CDC・ASME IDETC/CIE・RSJと、今回入れたPhysical AI Expo（TechEx）の
    2件が、この方針とぶつかる
...
HRSの実測値（2026-08-14時点）。`index.html` はフライト12・鉄道37・タクシー2、
机上用印刷版はフライト11で他は同数。`.place` 28件、`地図` の別リンク0件、
交通用の絵文字0件。393px・412px・1280pxで横あふれ0件、コンソールエラー0件、
机上用印刷版のスクリプト0件。

**2026-08-24に数え直した値。** `index.html` はフライト16・鉄道37・タクシー2、
机上用印刷版はフライト15。`.place` 43件。フライトと `.place` が増えているのは
その後の追記（旅程末尾へ移したカード、9/11の午後の追加など）によるもので、
方式が変わったわけではない。`地図` の別リンク0件、交通用の絵文字0件、
横あふれ0件、コンソールエラー0件、机上用印刷版のスクリプト0件は変わっていない。
...
検査は89件から103件へ（その後さらに増えて、2026-08-24時点は112件）。
数値はすべてEBの生成物を数え直した実測値である。
