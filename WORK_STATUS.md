...
- 概要の日程概要の組み立て（列挙＋代表1件＋時刻順）は、いまHRSの `build.mjs` にある。
  SIGGRAPH AsiaとLOGIMATで同じものを書くことになるので、`shared/trip-field` へ
  引き出す候補。EBの日程概要は合流までレーンが2本あるため、そのまま共通化はできない。
  先に「1日の行をどう組むか」だけを共通の関数にして、レーン割りはイベント側に残す
