...
気候の統計期間は都市ごとに違う（シュトゥットガルトは2015–2020、他は30年平年値）が、
これは未確認事項ではない。**値はいずれも出典を開いて確認済み**である。
季節感の用途では直近の値のほうが実態に近いこともあるので、期間の違いを
欠陥として扱わない。ページには出典名と期間を書き、それ以上の注記はしない。
