...
コミット `22451d8` の時点で入っている。**検証は通るが画面が正常ではない。**
詳細と直し方は `202609_HumanoidSummitEurope/CLAUDE_HANDOFF.md` の同名の節。

1. 家族印刷版の気候表が393pxで潰れる（見出しが1文字ずつ折り返す）
2. オンライン版の `.line-icon` が固定19pxで、13〜16pxの文字より大きい
...
1. 上の表示不具合2件を直す
2. HRSが固まったら、上の「HRSからEUROBLECHへ戻す」を実施する
